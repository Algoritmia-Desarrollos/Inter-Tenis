document.addEventListener('DOMContentLoaded', () => {
    // Al cargar la página, solo se llenan las opciones del primer filtro.
    ...
    // El renderizado principal se activa solo cuando el usuario elige un torneo.
...
async function populateTournamentFilter() {
    ...
    const { data: tournaments, error } = await supabase.from('tournaments').select('*').order('created_at', { ascending: false });
...
async function renderRankings() {
    ...
    // --- LÓGICA CORREGIDA ---
    // 1. Obtener los IDs de los jugadores del torneo.
    const { data: tournamentPlayersLinks, error: linksError } = await supabase
        .from('tournament_players')
        .select('player_id')
        .eq('tournament_id', tournamentId);

    if (linksError || !tournamentPlayersLinks) {
        container.innerHTML = '<p class="placeholder-text">Error al cargar los jugadores del torneo.</p>';
        return;
    }
    if (tournamentPlayersLinks.length === 0) {
    ...
    // 2. Obtener los detalles completos de esos jugadores.
    const { data: playersInTournament, error: playersError } = await supabase
        .from('players')
        .select('*, teams(name), categories(id, name)')
        .in('id', playerIds);

    // 3. Obtener los partidos completados de ese torneo.
    const { data: matchesInTournament, error: matchesError } = await supabase
        .from('matches')
        .select('*')
        .eq('tournament_id', tournamentId)
    ...
    if (playersError || matchesError) {
        container.innerHTML = '<p class="placeholder-text">Error al cargar los datos para el ranking.</p>';
        return;
    }
    ...
    container.innerHTML = ''; // Limpiar "Calculando..."
    ...
    stats.forEach((s, index) => {
        tableHTML += `
            <tr>
                ...
                <td>${s.pg}</td>
                <td>${s.pp}</td>
                <td><span class="diff-value">${s.difP > 0 ? '+' : ''}${s.difP}</span></td>
                <td class="divider-left">${s.sg}</td>
                <td>${s.sp}</td>
                <td><span class="diff-value">${s.difS > 0 ? '+' : ''}${s.difS}</span></td>
                <td class="divider-left">${s.gg}</td>
                <td>${s.gp}</td>
                <td><span class="diff-value">${s.difG > 0 ? '+' : ''}${s.difG}</span></td>
