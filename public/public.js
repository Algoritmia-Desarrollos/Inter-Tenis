...
const SUPABASE_URL_PUBLIC = 'https://vulzfuwesigberabbbhx.supabase.co';
const SUPABASE_ANON_KEY_PUBLIC = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ1bHpmdXdlc2lnYmVyYWJiYmh4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTMyMTExOTEsImV4cCI6MjA2ODc4NzE5MX0.5ndfB7FxvW6B4UVny198BiVlw-1BhJ98Xg_iyAEiFQw';
const supabasePublic = window.supabase.createClient(SUPABASE_URL_PUBLIC, SUPABASE_ANON_KEY_PUBLIC);
...
async function populateTournamentFilter() {
    ...
    const { data: tournaments } = await supabasePublic.from('tournaments').select('*').order('created_at', { ascending: false });
    if (tournaments) {
...
async function renderRankings() {
    ...
    if (playersError || matchesError) {
        container.innerHTML = '<p class="placeholder-text">Error al cargar los datos del torneo.</p>';
        return;
    }
